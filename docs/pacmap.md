...
### Scaled PCA Initialization + UMAP
...
but not range-scaled and I changed the PCA initialization in PaCMAP to scale
the standard deviation to 1 for all columns. Way, way up there I speculated that
an initialization like this would avoid the poor results I saw when I turned off
mid-pair weights.
